...
export class OwnerReports implements OnInit{private api=inject(ParkingBookingApiService);private cdr=inject(ChangeDetectorRef);bookings:StaffBooking[]=[];loading=true;error='';get revenue(){return this.bookings.reduce((sum,x)=>sum+(x.totalPrice??0),0)}get completed(){return this.bookings.filter(x=>x.status===2).length}ngOnInit(){this.api.getOwnerBookings().subscribe({next:data=>{this.bookings=data;this.loading=false;this.cdr.markForCheck()},error:e=>{this.error=e.message;this.loading=false;this.cdr.markForCheck()}})}}
